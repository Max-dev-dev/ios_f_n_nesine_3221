  ...
  Widget build(BuildContext context) {
    return BlocBuilder<FavoritesCubit, FavoritesState>(
      builder: (context, state) {
        final favoriteBlogs =
            state.favorites
                .where((fav) => fav['category'] == category)
                .map((fav) => fav['id'])
                .toSet();

        final filteredContent =
            content[category]!
                .where((blog) => favoriteBlogs.contains(blog['id']))
                .toList();

        if (filteredContent.isEmpty) {
          return const Center(
            child: Text(
              "No favorite blogs added yet.",
          ...
          itemBuilder: (context, index) {
            return Container(
              margin: EdgeInsets.only(
                bottom: index == filteredContent.length - 1 ? 135 : 10,
              ),
              padding: const EdgeInsets.all(16),
              decoration: BoxDecoration(
                color: Colors.white,
                  ...
                  Row(
                    ...
                    children: [
                      SizedBox(
                        width: 240,
                        child: Text(
                          filteredContent[index]['description']!,
                          style: const TextStyle(
                            fontSize: 12,
                            color: Colors.black,
                          ),
                            ...
                            MaterialPageRoute(
                              builder:
                                  (context) => BlogDetailScreen(
                                    id: filteredContent[index]['id']!,
                                    category: category,
                                    title: filteredContent[index]['title']!,
                                    description:
                                        filteredContent[index]['description']!,
                                    details:
                                        filteredContent[index]['details']!,
                                  ),
                            ),
                          );
                        },
